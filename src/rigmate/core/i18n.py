...
Vietnamese ('vi') is a supported locale.
Additional locales can be registered seamlessly.
"""
...
def t(key: str, locale: Optional[str] = None, **kwargs: Any) -> str:
    """
    Translate a key into localized string with interpolation support.
    
    Fallback chain:
    1. Specified locale (or active locale)
    2. Default locale ('en')
    3. The key itself if missing everywhere
    """
