"""Neutral diagnostic analysis logic decoupled completely from Blender, Pydantic, and external servers.

Pure Python standard library (re, datetime, typing).
Evaluates MeshInfo and ArmatureInfo contracts from rigmate.contracts.dto.
"""
...
from rigmate.localization.engine import t
...
class RigAnalyzer:
    ...
    def analyze(
        ...
        if mesh:
            ...
            # Check vertex weights summary
            if mesh.weights_summary:
                if mesh.weights_summary.unweighted_vertex_count > 0:
                    issues.append(
                        DiagnosticIssue(
                            code="WEIGHTS_UNWEIGHTED_VERTICES",
                            severity="WARNING",
                            title=t("diag.weights_unweighted_title", locale=locale, count=mesh.weights_summary.unweighted_vertex_count),
                            message=t("diag.weights_unweighted_msg", locale=locale, name=mesh.name, count=mesh.weights_summary.unweighted_vertex_count),
                            target_object=mesh.name,
                            suggested_action=t("diag.weights_unweighted_action", locale=locale),
                        )
                    )
                if mesh.weights_summary.exceeds_godot_max_weights or mesh.weights_summary.max_weights_per_vertex > 4:
                    issues.append(
                        DiagnosticIssue(
                            code="WEIGHTS_EXCEED_LIMIT",
                            severity="WARNING",
                            title=t("diag.weights_exceed_title", locale=locale, max_weights=mesh.weights_summary.max_weights_per_vertex),
                            message=t("diag.weights_exceed_msg", locale=locale, name=mesh.name, max_weights=mesh.weights_summary.max_weights_per_vertex),
                            target_object=mesh.name,
                            suggested_action=t("diag.weights_exceed_action", locale=locale),
                        )
                    )
        ...
        return RigDiagnosticReport(
            ...
            ready_for_godot=(error_count == 0 and warning_count == 0),
